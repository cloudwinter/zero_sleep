// component/tab/weitiao/weitiao-W7.js
...
Component({
  ...
  data: {
    ...
    startTime: '',
    endTime: '',
    beibutzLeftTop: false,
    beibutzLeftBottom: false,
    beibutzRightTop: false,
    beibutzRightBottom: false,
    tuibutzLeftTop: false,
    tuibutzLeftBottom: false,
    tuibutzRightTop: false,
    tuibutzRightBottom: false,
    beibutzTop: false,
    beibutzBottom: false,
    tuibutzTop: false,
    tuibutzBottom: false,
  ...
  lifetimes: {
    created: function () {
      // 在组件实例刚刚被创建时执行
      console.info("weitiao-W8-->created");
      var that = this;
  ...
  methods: {
    ...
    initConnected(connected) {
      ...
      console.info('weitiao-W8->initConnected:', connected, this.observer);
      that.setData({
        connected: connected,
      })
    ...
    touchStart(e) {
      ...
      if (type == 'beibutzTop') {
        ...
        this.donghua(true, this);
        this.tapFentiBeibutz(true, true, true);
      } else if (type == 'beibutzLeftBottom') {
        this.setData({
          currentAnjian: {
            anjian: 'beibutz',
            name: '背部调整'
          },
          beibutzLeftBottom: true
        });
        this.donghua(false, this);
        this.tapFentiBeibutz(false, true, true);
      } else if (type == 'beibutzRightTop') {
        this.setData({
          currentAnjian: {
            anjian: 'beibutz',
            name: '背部调整'
          },
          beibutzRightTop: true
        });
        this.donghua(true, this);
        this.tapFentiBeibutz(true, true, false);
      } else if (type == 'beibutzRightBottom') {
        this.setData({
          currentAnjian: {
            anjian: 'beibutz',
            name: '背部调整'
          },
          beibutzRightBottom: true
        });
        this.donghua(false, this);
        this.tapFentiBeibutz(false, true, false);
      }else if (type == 'tuibutzLeftTop') {
        this.setData({
          currentAnjian: {
            anjian: 'tuibutz',
            name: '腿部调整'
        ...
        this.donghua(true, this);
        this.tapFentiTuibutz(true, true, true);
      } else if (type == 'tuibutzLeftBottom') {
        this.setData({
          currentAnjian: {
            anjian: 'tuibutz',
            name: '腿部调整'
          },
          tuibutzLeftBottom: true
        });
        this.donghua(false, this);
        this.tapFentiTuibutz(false, true, true);
      } else if (type == 'tuibutzRightTop') {
        this.setData({
          currentAnjian: {
            anjian: 'tuibutz',
            name: '腿部调整'
          },
          tuibutzRightTop: true
        });
        this.donghua(true, this);
        this.tapFentiTuibutz(true, true, false);
      } else if (type == 'tuibutzRightBottom') {
        this.setData({
          currentAnjian: {
            anjian: 'tuibutz',
            name: '腿部调整'
          },
          tuibutzRightBottom: true
        });
        this.donghua(false, this);
        this.tapFentiTuibutz(false, true, false);
      }
    },
    /**
     * 抬起事件
    ...
    touchEnd(e) {
      ...
      if (type == 'beibutzTop') {
        ...
        this.tapFentiBeibutz(true, false, true);
      } else if (type == 'beibutzLeftBottom') {
        this.setData({
          beibutzLeftBottom: false
        });
        this.tapFentiBeibutz(false, false, true);
      } else if (type == 'beibutzRightTop') {
        this.setData({
          beibutzRightTop: false
        });
        this.tapFentiBeibutz(true, false, false);
      } else if (type == 'beibutzRightBottom') {
        this.setData({
          beibutzRightBottom: false
        });
        this.tapFentiBeibutz(false, false, false);
      }else if (type == 'tuibutzLeftTop') {
        this.setData({
          tuibutzLeftTop: false
        });
        this.tapFentiTuibutz(true, false, true);
      } else if (type == 'tuibutzLeftBottom') {
        this.setData({
          tuibutzLeftBottom: false
        });
        this.tapFentiTuibutz(false, false, true);
      } else if (type == 'tuibutzRightTop') {
        this.setData({
          tuibutzRightTop: false
        });
        this.tapFentiTuibutz(true, false, false);
      } else if (type == 'tuibutzRightBottom') {
        this.setData({
          tuibutzRightBottom: false
        });
        this.tapFentiTuibutz(false, false, false);
      }
    },
     ...
     * @param {*} left 左边
     */
    tapFentiBeibutz(top, start, left) {
      var cmd = '';
      if (!start) {
        // 松开 发停止码
        cmd = '0000D700';
     ...
     * @param {*} left 左边
     */
    tapFentiTuibutz(top, start, left) {
      var cmd = '';
      if (!start) {
        // 松开 发停止码
        cmd = '0000D700';
