// component/kuaijie/kuaijie-K6.js
...
Component({
  ...
  lifetimes: {
    created: function () {
      // 在组件实例刚刚被创建时执行
      console.info("kuaijie-k6-->created");
      var that = this;
    ...
    detached: function () {
      // 在组件实例被从页面节点树移除时执行
      console.info("kuaijie-k6-->detached");
      var that = this;
  ...
  methods: {
    ...
    initConnected(connected) {
      ...
      console.info('kuaijie-K6->initConnected:', connected, this.observer);
      that.setData({
        connected: connected,
      })
    ...
    blueReply(cmd) {
      var that = this.observer;
      console.error('kuaijie-K6->blueReply',cmd);
      cmd = cmd.toUpperCase();
      ...
      console.info('kuaijie-k6->askBack', cmd, prefix);
