...
Page({
  ...
  blueReply(cmd) {
    ...
    if (cmd.indexOf('FFFFFFFF01000E14') >= 0) {
      ...
      let fsfdCmd = cmd.substr(30, 2);
      let fsfdLow = fsfdCmd == '01' ? true : false;
      let fsfdMiddle = fsfdCmd == '02' ? true : false;
      let fsfdHigh = fsfdCmd == '03' ? true : false;
  ...
  fanshenTap: function (e) {
    var type = e.currentTarget.dataset.type;
    if (type == 'low') {
      this.setData({
        fsfdLow: !this.data.fsfdLow,
        fsfdMiddle: false,
        fsfdHigh: false
      })
    } else if (type == 'middle') {
      this.setData({
        fsfdMiddle: !this.data.fsfdMiddle,
        fsfdLow: false,
        fsfdHigh: false
      })
    } else if (type == 'high') {
      this.setData({
        fsfdHigh: !this.data.fsfdHigh,
        fsfdLow: false,
        fsfdMiddle: false
      })
    }
  ...
  saveTap: function () {
    ...
    if(this.data.fsfdLow) {
      cmd = cmd + '01'
    } else if(this.data.fsfdMiddle) {
      cmd = cmd + '02'
    } else if(this.data.fsfdHigh) {
      cmd = cmd + '03'
    } else {
      cmd = cmd + '00'
    }
    ...
    setTimeout(() => {
      // 返回上一页
      var cmdNext = 'FFFFFFFF0100070B01';
      let cmdCrcNext = crcUtil.HexToCSU16(cmdNext);
      cmdNext = cmdNext + cmdCrcNext;
      util.sendBlueCmd(connected, cmdNext);
    }, 200);

    setTimeout(() => {
      // 再发码
      var cmdNext = 'FFFFFFFF01000E0B00';
      let cmdCrcNext = crcUtil.HexToCSU16(cmdNext);
      cmdNext = cmdNext + cmdCrcNext;
      util.sendBlueCmd(connected, cmdNext);
    }, 400);
